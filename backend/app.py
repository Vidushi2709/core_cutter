...
# Add parent directory to path for alert_system imports
sys.path.insert(0, str(Path(__file__).parent.parent))

from main import PhaseBalancingController
from utility import DataStorage
from alert_system.alert_manager import get_alert_manager
...
class PhaseAnalytics(BaseModel):
    ...
    avg_voltage: float
    total_current: float  # Total current for this phase
    house_count: int
...
alert_manager = get_alert_manager()  # Initialize alert manager
        ...
        for ps in phase_stats:
            ...
            for house_id, house in controller.registry.houses.items():
                if house.phase == ps.phase and house.last_reading:
                    ...
                    total_current += abs(reading.current)
...
def get_phase_details(phase: str) -> PhaseAnalytics:
    ...
    try:
        ...
        return PhaseAnalytics(
            ...
            total_current=sum(abs(h.current) for h in houses_on_phase),
...
def root():
    return {
        ...
        "endpoints": {
            ...
            "alerts": {
                "history": "GET /alerts/history - Get alert history",
                "active": "GET /alerts/active - Get active (unresolved) alerts",
                "test": "POST /alerts/test - Send test alert"
            },
...
def get_alert_history(limit: int = 50, unresolved_only: bool = False):
    """
    Get alert history.
    ...
    Returns:
        List of alert events with details
    """
    try:
        alerts = alert_manager.get_alert_history(limit=limit, unresolved_only=unresolved_only)
        return {
            "count": len(alerts),
            "alerts": alerts
        }
...
def get_active_alerts():
    ...
    Returns:
        List of active alerts that need attention
    """
    try:
...
def test_alert_system():
    ...
    Use this to verify that email, SMS, and webhook notifications are working correctly.
    
    Returns:
        Status of test alerts sent through each channel
    """
    try:
